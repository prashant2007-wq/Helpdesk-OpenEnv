...
def main() -> None:
    ...
    for task_id in ["triage_easy", "triage_medium", "triage_hard"]:
        ...
        if task_id == "triage_easy":
            _, rew = env.step(Action(route_to_team=Team.IT_SUPPORT, set_priority=Priority.P2, submit=True))
        elif task_id == "triage_medium":
            env.step(
                Action(
                    ask_clarifying_question="What time did you see this and did you approve any MFA prompts?"
                )
            )
            _, rew = env.step(Action(route_to_team=Team.SECURITY, set_priority=Priority.P1, submit=True))
        else:
            _, rew = env.step(
                Action(
        ...
        score = float(rew.info.get("final_score", "0.0"))
